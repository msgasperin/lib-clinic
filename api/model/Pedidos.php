<?php
  //print_r($sql->errorInfo());
	...
	class Pedidos extends Conexion {
		...
		// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ FUNCIONES pedidos ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		public function obtiene_pedidos_fecha(string $fecha_ini, string $fecha_fin) {
			$res = [];
			try {
				$sql = $this->dbh->prepare("SELECT id_pedido, tipo_pedido, folio, nom_cliente_hist, logo, id_categoria_cliente_fk, nom_categoria_cliente_hist, icon_categoria, id_lista_precios_fk, subtotal, descuento, cargo_extra, motivo_cargo_extra, total, costo_total, utilidad_total, abonos, debiente, fecha_entrega, DATE_FORMAT(fecha_entrega,'%d-%m-%Y') AS fecha_entrega_format, fecha_limite_pago, DATE_FORMAT(fecha_limite_pago,'%d-%m-%Y') AS fecha_limite_pago_format, fecha_cap, DATE_FORMAT(fecha_cap,'%d-%m-%Y %H:%i:%s') AS fecha_cap_format, P.user_cap, DATE_FORMAT(fecha_surtido,'%d-%m-%Y %H:%i:%s') AS fecha_surtido_format, DATE_FORMAT(fecha_pagado,'%d-%m-%Y %H:%i:%s') AS fecha_pagado_format, P.estatus, observaciones, key_query
				FROM pedido AS P 
				...
				WHERE fecha_cap >= ? AND fecha_cap <= ? AND P.estatus <> ?");
				$sql->execute(array($fecha_ini, $fecha_fin, 'Demo'));
				$res = $sql->fetchAll(PDO::FETCH_ASSOC);
			} catch (Exception $error) {
        		error_log($error->getMessage());
			}
