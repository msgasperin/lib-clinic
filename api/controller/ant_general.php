<?php
  require_once('../model/AntGeneral.php');
  require_once('../model/Globales.php');
  $v = new AntecedenteGeneral();
  $g = new Globales();
  ...
  if(isset($_SESSION["id_usuario"]) && $_SESSION["id_usuario"] != '') {
    if(isset($_POST['func'])) {
      switch ($_POST['func']) {
        ...
        case 'obtiene_antecedentes_generales':
          ...
          $res = $v->obtiene_antecedentes_generales($_POST["idPaciente"]);
          echo json_encode(["estatus" => 200, "mensaje" => "", "data" => $res]);
          
        break;

        case 'guardar_antecedentes_generales':
          ...
          if($resVal) {
            $res = $v->actualizar_antecedentes_generales($_POST, $_SESSION["nombre"]);
            $mensaje = 'Antecedentes generales actualizados del paciente: '.$_POST["nomPaciente"];
          }
          else {
            $res = $v->inserta_antecedentes_generales($_POST, $_SESSION["nombre"]);
            $mensaje = 'Antecedentes generales registrados del paciente: '.$_POST["nomPaciente"];
          }          
