<?php
  require_once('../model/AntPatologicos.php');
  require_once('../model/Globales.php');
  $v = new AntecedentePatologico();
  $g = new Globales();
  ...
  if(isset($_SESSION["id_usuario"]) && $_SESSION["id_usuario"] != '') {
    if(isset($_POST['func'])) {
      switch ($_POST['func']) {
        ...
        case 'obtiene_antecedentes_patologicos':
          ...
          $res = $v->obtiene_antecedentes_patologicos($_POST["idPaciente"]);
          echo json_encode(["estatus" => 200, "mensaje" => "", "data" => $res]);
          
        break;

        case 'guardar_antecedentes_patologicos':
          ...
          if($resVal) {
            $res = $v->actualizar_antecedentes_patologicos($_POST, $_SESSION["nombre"]);
            $mensaje = 'Antecedentes patológicos actualizados del paciente: '.$_POST["nomPaciente"];
          }
          else {
            $res = $v->inserta_antecedentes_patologicos($_POST, $_SESSION["nombre"]);
            $mensaje = 'Antecedentes patológicos registrados del paciente: '.$_POST["nomPaciente"];
          }          
